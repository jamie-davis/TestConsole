...
namespace TestConsole.OutputFormatting.Internal
{
    ...
    internal static class TabularReport
    {
        ...
        public static IEnumerable<string> Format<T, TChild>(IEnumerable<T> data, IEnumerable<ColumnFormat> columns,
            int width, SplitCache cache, int numRowsToUseForSizing = 0, ReportFormattingOptions options = ReportFormattingOptions.Default, string columnDivider = null, IEnumerable<BaseChildItem<TChild>> childReports = null)
        {
            ...
            using (new TempCulture(culture))
            {
                    ...
                    foreach (var l in DoFormat(data, columns, width, numRowsToUseForSizing, 4, options, columnDivider, statistics, childReports, cache))
                        yield return l;
            }
        }
        ...
        private static IEnumerable<string> DoFormatFromCachedRows<T, TChild>(CachedRows<T> data, IEnumerable<ColumnFormat> specifiedColumns, int width, int numRowsToUseForSizing, int tabLength, ReportFormattingOptions options, string columnDivider, Statistics statistics, IEnumerable<BaseChildItem<TChild>> childReports, SplitCache cache)
        {
        ...
        private static IEnumerable<string> DoFormat<T, TChild>(IEnumerable<T> data, IEnumerable<ColumnFormat> specifiedColumns, int width, int numRowsToUseForSizing, int tabLength, ReportFormattingOptions options, string columnDivider, Statistics statistics, IEnumerable<BaseChildItem<TChild>> childReports, SplitCache cache)
        {
            var columns = FormatAnalyser.Analyse(typeof(T), specifiedColumns, IncludeAllColumns(specifiedColumns, options));
            Action<ColumnWidthNegotiator, T> addRowAction = (sz, item) => sz.AddRow(item);

            return FormatFromDataFeed(data, width, numRowsToUseForSizing, tabLength, columns, addRowAction, options, columnDivider, statistics, childReports, cache);
        }
        ...
        private static IEnumerable<string> FormatFromDataFeed<T, TChild>(IEnumerable data, int width, int numRowsToUseForSizing, 
            ...
            IEnumerable<BaseChildItem<TChild>> childReports, SplitCache cache)
        {
            ...
            foreach (var l in ReportRowsUsedForSizing<T, TChild>(columns, sizer, tabLength, columnDivider, widths, sizeStats, childReports, headingsControl, cache))
                yield return l;
            ...
            if (enumeratorValid)
            {
                do
                {
                    ...
                    foreach (var l in ReportRow<T, TChild>(dataEnumerator.Current, columns, sizer, tabLength, columnDivider, widths, reportStats, childReports, headingsControl, cache))
                        yield return l;
        ...
        private static IEnumerable<string> ReportRow<T, TChild>(object item, List<PropertyColumnFormat> columns, ColumnWidthNegotiator sizer, int tabLength, string columnSeparator, int[] widths, Statistics statistics, IEnumerable<BaseChildItem<TChild>> children, HeadingManager headingsControl, SplitCache cache)
        {
            var rowValues = columns
                .Select(c => GetColValue(item, c, cache))
                .ToList();
            foreach (var l in ReportRowValues<T, TChild>(columns, sizer, tabLength, columnSeparator, item, rowValues, widths, statistics, children, headingsControl, cache))
                yield return l;
        }

        private static FormattingIntermediate GetColValue(object item, PropertyColumnFormat c, SplitCache cache)
        {
            object value;
            if (c.Property == null)
                value = item;
            ...
            if (c.Format.Type == typeof (IConsoleRenderer) && value is IConsoleRenderer)
                return new FormattingIntermediate(value as IConsoleRenderer, cache);

            return new FormattingIntermediate(ValueFormatter.Format(c.Format, value), cache);
        }

        private static IEnumerable<string> ReportRowsUsedForSizing<T, TChild>(List<PropertyColumnFormat> columns, ColumnWidthNegotiator sizer, int tabLength, string columnSeparator, int[] widths, Statistics statistics, IEnumerable<BaseChildItem<TChild>> children, HeadingManager headingsControl, SplitCache cache)
        {
            foreach (var rowValues in sizer.GetSizingValues())
            {
                var stats = new Statistics();
                var columnValues = rowValues.GetValues().ToList();
                foreach (var l in ReportRowValues<T, TChild>(columns, sizer, tabLength, columnSeparator, rowValues.RowItem, columnValues, widths, stats, children, headingsControl, cache))
                    yield return l;
                statistics.WordWrapLineBreaks += stats.WordWrapLineBreaks;
            }
        }

        private static IEnumerable<string> ReportRowValues<T, TChild>(List<PropertyColumnFormat> columns, ColumnWidthNegotiator sizer, int tabLength, string columnSeparator, object rowItem, IList<FormattingIntermediate> rowValues, int[] widths, Statistics statistics, IEnumerable<BaseChildItem<TChild>> children, HeadingManager headingsControl, SplitCache cache)
        {
            ...
            var wrappedValues = columns
                ...
                {
                    int wrappedLines;
                    var lines = RenderCol(tabLength, rowValues[i], c, cache, out wrappedLines);
                ...
                {
                    int wrappedLines;
                    var lines = StackedColumnValues(rowValues, sizer, tabLength, cache, out wrappedLines);
                    maxLineBreaks = Math.Max(maxLineBreaks, wrappedLines);
                    return lines;
                })())
                .ToArray();
            ...
            yield return ReportColumnAligner.AlignColumns(widths, wrappedValues, ColVerticalAligment.Top, columnSeparator);
            ...
            if (children != null)
            {
                ...
                {
                    var childOutput = PrefixLines.Do(baseChildItem.Render(rowItem, sizer.BaseWidth - 4, cache), "    ");
        ...
        private static string[] RenderCol(int tabLength, FormattingIntermediate rowValue, PropertyColumnFormat c, SplitCache cache, out int wrappedLines)
        {
            if (rowValue.RenderableValue == null)
            {
                return ColumnWrapper.WrapAndMeasureValue(rowValue.TextValue, c.Format, c.Format.ActualWidth, cache, tabLength, 0, out wrappedLines);
            }
        ...
        private static IEnumerable<string[]> StackedColumnValues(IEnumerable<FormattingIntermediate> rowValues, ColumnWidthNegotiator sizer, int tabLength, SplitCache cache, out int wrappedLines)
        {
            ...
            var formattedLines = PropertyStackColumnFormatter.Format(sizer.StackedColumns, rowValues.Skip(sizer.Columns.Count).ToList(),
                sizer.StackedColumnWidth, cache, tabLength)
                .ToArray();
